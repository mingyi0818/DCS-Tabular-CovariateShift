...
def dcs_selection(X_train, X_test, n_select, n_clusters=50, method='logistic', seed=42):
    ...
    Algorithm:
      ...
      3. Allocate selection budget proportional to cluster density ratio
    ...
    # Step 4: Allocate budget proportional to cluster ratio (with min 1 per cluster)
    # Use ratio * size as weight (total mass of "test-similar" samples)
    ...
    # Adjust total to exactly n_select
    while allocation.sum() > n_select:
        # Remove from cluster with smallest weight
        c_min = np.argmin(cluster_weights * (allocation > 1))
        allocation[c_min] -= 1
    while allocation.sum() < n_select:
        # Add to cluster with largest weight
